...
export default function CollectionDetails() {
  ...
  const itemQuery = useQuery({
    ...
    queryFn: () => apiFetch<any>(`/collections/${id}`),
    enabled: !!id
  });
  const updatesQuery = useQuery({
    queryKey: ['collection-updates', id],
    queryFn: () => apiFetch<any[]>(`/collections/${id}/updates`),
    enabled: !!id
  });

  if (itemQuery.isLoading) return <BreachSkeleton className="h-28" />;
  if (itemQuery.isError) return <BreachErrorState title="Collection offline" />;
  ...
  return (
    <div className="space-y-4">
      <BreachCard className="space-y-2">
        ...
        <div className="text-xs text-text1">Last change: {formatDate(item.lastChangeAt)}</div>
      </BreachCard>
          ...
          {updatesQuery.data.map((update) => (
            <BreachCard key={update.id} className="space-y-2">
              ...
              <div className="text-sm text-text1">
                {update.changeJson?.next?.title ? `Title: ${update.changeJson.next.title}` : 'Change detected'}
              </div>
            </BreachCard>
          ))}
        </div>
